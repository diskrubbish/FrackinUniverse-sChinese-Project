...
def filelist(project_id, Authorization):
    return json.loads(
        requests.get(base_url + project_id + "/files", headers={"Authorization": Authorization}).content)
...
def file(project_id, Authorization, fileid=None ,page=None, pageSize=50):
    params = {"pageSize": pageSize, "page": page,
                  "file": fileid}
    return json.loads(
        requests.get(base_url + project_id + "/strings", params = params ,headers={"Authorization": Authorization},).content)["results"]
