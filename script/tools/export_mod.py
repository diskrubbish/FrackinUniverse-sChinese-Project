...
translations_dir = "F:/FrackinUniverse-sChinese-Project-FAST/translations"
mod_dir = "mods"
...
def dump_patch(items,translation,patchfiles):
...
for subdir, dirs, files in walk(translations_dir):
    for thefile in files:
        ...
        for label in jsondata:
            if "Chs" not in label["Texts"]:
                continue
            if isinstance(label["Texts"]["Chs"],str):
                if len(label["Texts"]["Chs"]) == 0:
                    continue
                ...
                dump_patch(label["Files"].items(),translation,patchfiles)
            elif isinstance(label["Texts"]["Chs"],dict):
                file_list = list()
                ...
                file_list = enumerate(file_list)
                list_len = len(label["Files"].items())
                if "0" in label["Texts"]["Chs"].keys():
                   add_count(labelsTranslated, filename, 1) 
                for index,files in file_list:
                    if str(index+1) in label["Texts"]["Chs"].keys():
                        add_count(labelsTranslated, filename, 1)
                        translation = label["Texts"]["Chs"][str(index+1)]
                        dump_patch([tuple([files[0],[files[1]]])],translation,patchfiles)
                    else:
                        if str(0) in label["Texts"]["Chs"].keys():
                            translation = label["Texts"]["Chs"]["0"]
                            dump_patch([tuple([files[0],[files[1]]])],translation,patchfiles)
